...
async function main() {
    // STEP 1 ===================================
    console.log(`STEP 1 ===================================`);
    const bytecode = fs.readFileSync("./Demo_sol_Demo.bin");
    console.log(`- Done \n`);
    ...
    const tB1 = await bCheckerFcn(operatorId);
    console.log(`- Treasury balance: ${tB1} units of token ${tokenId}`);
    ...
    console.log(
        `- Contract balance: ${cCheck.tokenRelationships._map.get(tokenId.toString()).balance.low
    ...
    const contractExecTx3 = await new ContractExecuteTransaction()
        ...
        .freezeWith(client);
    const contractExecSign3 = await contractExecTx3.sign(operatorKey);
    const contractExecSubmit3 = await contractExecSign3.execute(client);
    ...
    console.log(`- Token transfer from contract to Operator: ${contractExecRx3.status.toString()}`);

    const tB2 = await bCheckerFcn(operatorId);
    console.log(`- Treasury balance: ${tB2} units of token ${tokenId}`);

    const cCheck2 = await new ContractInfoQuery().setContractId(contractId).execute(client);
    console.log(
        `- Contract balance: ${cCheck2.tokenRelationships._map.get(tokenId.toString()).balance.low
        } units of token ${tokenId}`
    );
